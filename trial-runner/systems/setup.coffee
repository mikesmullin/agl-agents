...
export setupSystem = ->
  ...
  for file in yamlFiles
    ...
    # correctAnswer: prefer _parsed_valid_operation (deterministic), fall back to recommendation.operations
    correctAnswer = String(
      entity.operator_input?._parsed_valid_operation or
      entity.recommendation?.operations or ''
    ).trim()
    ...
    # Build trial entity: preserve most archive fields; strip only the components that must be
    # re-generated by the trial pipeline.  Keeping fingerprint, summary, content, envelope,
    # recall.presentationCandidate, etc. avoids redundant LLM calls and lets us focus the trial
    # on the recall→recommend→coach loop.
    #
    # Components stripped (must be regenerated each trial run):
    #   recall.journalContext  – re-run by recallJournalSystem
    #   retrospective          – re-run by recommendSystem
    #   recommendation         – re-run by recommendSystem
    #   traces                 – timing from a different run; irrelevant
    #   execution              – from original live run; not applicable in trial mode
    #   journal                – from original live run; not applicable in trial mode
    #   plan                   – from original live run; not applicable in trial mode
    #   apply                  – from original live run; not applicable in trial mode
    STRIP_TOP = new Set ['retrospective', 'recommendation', 'traces', 'execution', 'journal', 'plan', 'apply']

    trialEntity = {}
    for own key, value of entity
      if key is 'recall'
        # Strip journalContext only; preserve presentationCandidate + usePresentationPreferences
        recallRest = { ...(value or {}) }
        delete recallRest.journalContext
        trialEntity.recall = recallRest if Object.keys(recallRest).length > 0
      else if not STRIP_TOP.has key
        trialEntity[key] = value

    # Always write _trial fields (overrides any existing _trial from archive)
    trialEntity._trial =
