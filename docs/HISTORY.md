...
# day 9: improving trial-runner agent

- read `**/README.md` to understand this project

let's improve the `trial-runner/`
- we can save time; we don't need to run as many systems as in personal-email/agent.coffee
  - trial-runner/agent.coffee:122~126: comment-out these system invocations
- likewise, when the entity yaml files are copied into the `trial/{{trial_id}}/` folder
    - and they are copied-into because they are recordings from when human-operator + personal-email/agent were working togther to successfully triage the very same email
      - while trial-runner is merely practicing its own recommendation, on the offline/recorded copy (in a mock trial run)
  - we don't need to truncate them to just `{id, origin.raw}`
    - we can keep more of the component > fields, which will permit us to skip [re-]running specific systems (which generated those outputs previously)
    - split the `recallSystem()` into 
      - two systems
        - `recallJournal`: writes `recall.journalContext`
        - `recallPresentation`: writes `recall.{presentationCandidate,usePresentationPreferences}`
      - this way the `recallPresentation` does not have to be re-run by the `trial-runner` (it's only run by `personal-email/agent.coffee`)
        - instead we only [re-]run `recallJournal()` system from trial-runner/agent loop

    - we can keep most components in the entity yaml
      - and only remove `{recall.journalContext,retrospective,recommendation,traces,execution,journal,plan,apply}` keys
        - this way the trial-runner/agent loop can be more like:
          - loadSystem: (skip?)
          - fingerprintSystem: skip (the component data is already preserved from when personal-email/agent processed the email entity)
          - seedJournalSystem: skip (the component data is already preserved from when personal-email/agent processed the email entity)
          - recallJournalSystem: keep (this is one of the main tasks of trial-runner; to see if it can recall the CORRECT journal entry, which generates the CORRECT recommendation)
          - recallPresentationSystem: skip
          - summarizeSystem: skip (the component data is already preserved from when personal-email/agent processed the email entity)
          - recommendSystem: keep (this is one of the main tasks of trial-runner; to see if it can produce the CORRECT recommendation)
          - displaySystem: skip (the output component data of this system isn't likely to be used, since it just an offline-mode/training-trial)
          - operatorSystem: keep
              - once a recommended is generated
                - we simply want to know if recommendation was correct
                  - if yes, move to next email
                  - if no, then involve seance + coach
          - seanceSystem: keep
          - coachSystem: keep

- there is a particular flow i want for trial-runner
  - recallJournalSystem: load journal entry
  - recommendSystem: make recommendation
  - operatorSystem: compare recommendation to recorded answer--if correct, email is done being processed (we have concluded/proven that the correct answer can be generated by an llm, given urrent configuration == successful email trial!)
  - seanceSystem: divine hint as to why llm failed, and how to correct it
  - coachSystem: apply appropriate change(s) to journal
  - (send the email back to the beginning of the loop)
    - ie. remove `{recall.journalContext,retrospective,recommendation,traces,execution,journal,plan,apply}` keys from the entity yaml file
      - this will cause it to be picked-up and re-processed from the point of recallJournalSystem
        - but this time it will load the modified journal entry made by coachSystem
          - and the goal is to see that operatorSystem finds the email recommendation was successfully generated
            - otherwise, it returns to the coachSystem (as often as necessary--there is no limit in terms of coach visits/cycles. as human operator, i will simply end any trial that runs for too long of time)
- but i am trying to reuse as much of personal-email/ code as i can (to keep code DRY)
  - because trial-runner is like a kind of unit test for personal-email


- critically, from the entity yaml data point of view
  - the primary questions -> fields of interest (to trial-runner/agent) are:
    - Q: does it load correct journal entry?
      - recall.journalContext
      - recommendation.journal_id
    - Q: does it make the correct recommendation?
      - recommendation.operations
        - CRITICAL: the format of this field should be deterministically validated 
          - against these patterns (there are only 4 valid response patterns)
            - `move to {{valid_folder}`: will move email to a specific folder
              - its important that the valid_folder matches the official list defined in (personal-email/config.yaml:39~64)
            - `delete`: will delete the email
            - `archive`: will archive the email
            - `skip`: used when confidence is too low
          - the result of failing to validate
            - this represents a problem with the microagent `01-recommend-action`
              - its not a problem that coach can fix (at least right now)
              - its something the human operator (and his trusty AI Agent assistant, Claude/Copilot) need to fix
                - usually with the system prompt, user prompt, or tool calls (their descriptions, etc. (the text that LLM can see))
                  - essentially, the tool calls need to reject back to the LLM if the LLM has not produced a valid output
                    - and ask the LLM to try again (in an infinite loop, if need be)
                     - until a valid answer is generated (ie. matches our expected format)
              - so the trial run should just end immediately
                - with an error about which email failed (share the email.id) in stderr
                  - also we should be able to provide a cli param to trial-runner/agent.coffee, so we can restart/re-run from/a specific email id
                    - this way human/claude could keep tweaking the microagent code and re-running trial-runner/agent
                      - until it is finally passing validation, again


        - if the recommendation is valid format
          - but does NOT match the answer the human operator gave
            - IMPORTANT: how we judge correctness here
              - (one-time problem i need you to fix, in the data)
               - currently its subjective
                 - `operator_input.instruction`
                   - can sometimes contain a paragraph
                     - it might contain words like `move to Stock`
                       - but it'll be buried somewhere in that paragraph
                         - so it requires an llm to read and decide (hopefully its interpretation is correct; but it can be wrong sometimes)
               - but i want it to become deterministic
                 - i want there to be a field in `operator_input` called `operator_input._parsed_valid_operation`
                   - this will contain one of the 4 possible response patterns which is passing validation
                     - but it will be provided (either: (by the human operator writing to that field directly), or (by an llm parsing the human `operator_input.instruction` and generating (+ validated) that (this is most likely; but note its similar-to but separate-from the recommendation llm which does a similar thing)))                    
               - here's how we can implement the one-time fix
                 - write a temporary script
                   - run through all the `_archive/*.yaml`
                     - have the llm/microagent ((a prototype of) the one responsible for converting `operator_input.instruction` -> `operator_input._parsed_valid_operation`)
                       - generate the `operator_input._parsed_valid_operation`
                         - save it to the file
                           - now, going forward
                             - trial-runner/agent can simply compare those two fields (string equality; NOTE: validation step should also normalize the output to all-lowercase (except folder names are case-sensitive))
                               - `recommendation.operations == operator_input._parsed_valid_operation`
                                 - now its deterministic! easy to check/verify during trial runs

- so, in summary
  - trial runner is primarily 
    - responsible for
      - amending journal entries
      - until a valid answer is generated by the recommend microagent
    - and doing this on a per-email basis
    - until all emails are passing

  - which means
    - it proves
      - we have a system that can
        - correctly triage my email inbox
          - without human supervision
            - this is the primary goal/value

help me make these changes to improve our trial-runner/

...

- i also need a way to track/indicate progress from trial-runner/agent
  - i am thinking a file should be created in the `trial/{{id}}/` folder
    - like `progress.yaml`
      - it should include as many useful metrics as possible
        - for me to understand how well we are progressing
          - ie. if i have 800 emails to process, and that could take hours
            - its important to be able to answer questions like:
              - how far along are we?
                - how many emails have been processed?
                  - how many of those succeeded on first try? vs. needed subsequent retries? (or how many tries were necessary for before the email passed?)
                  - whats the pass/fail rate overall?
                    - how many emails have passed?
                      - failed?
                        - how many times is coach having to be involved on min/max/mean before an email succeeds?
              - what's the time to process one email (one round) (min/max/mean)?
                - whats the remaining time, based on that estimate?
              - log[]
                - latest advice/change emitted by coach (entries)
                  - email id + advice (this way while i am bored, i can look at individual email yaml cases, and see if they are progressing in a way that makes sense to me as human operator (subjectively))
                    - this way i can stop the trial run if it seems to be stuck or not going in a good direction
    - and then in the `email-trainer/` > Training section (sidebar menu) > there should be a new page called "Trial Run"
      - which can not only start/stop the `bun trial-runner/agent.coffee` process (from the web ui)
       - but can see how long the trial runner has been running (ie. a .lock file ensures only one trial-runner is running at-a-time)
       - and can use this `progess.yaml` file
         - to show a dashboard encapsulating all of those named useful metrics
           - so i as a human operator can oversee the progress of the trial runner
