...
export const spawn = _G.spawn = (cmd, args = [], options = {}) => {
  const { stdio = 'pipe', assertExit0 = false, scope = 'agent' } = options;
  const settledResult = () => ({
    cmd: result.cmd,
    code: result.code,
  ...
  const proc = _spawn(cmd, args, { stdio });
  ...
  result.promise = new Promise((resolve, reject) => {
    ...
    proc.on('error', (err) => {
      debugLog(scope, 'spawn.error', { cmd: result.cmd, error: err?.message || String(err) });
      reject(err);
    });
  });
