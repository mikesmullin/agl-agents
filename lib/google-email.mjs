...
export const extractEmailEnvelope = _G.extractEmailEnvelope = (emailYamlText) => {
  const tryParse = (text) => {
    try {
      const parsed = YAML.parse(String(text || '').replace(/[^\x00-\x7F]/g, ''));
  ...
  const result = tryParse(emailYamlText);
  ...
  const stripped = String(emailYamlText || '').replace(/^body:[\s\S]*/m, '');
...
_G.emailRead = async (emailId) => _G.spawn('google-email', ['inbox', 'read', emailId]);

_G.emailUnread = async (emailId) => _G.spawn('google-email', ['inbox', 'unread', emailId]);

_G.emailDelete = async (emailId) => _G.spawn('google-email', ['delete', emailId]);
...
_G.emailMove = async (emailId, folder) => {
  ...
  return _G.spawn('google-email', ['move', emailId, folder]);
};
