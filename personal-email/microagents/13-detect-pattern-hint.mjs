...
_G.detectPatternHintMicroagent = async (emailText, instruction) => {
  return _G.traceStep('🔁', 'Detecting pattern hint', async () => {
    const microagent = await Agent.factory({
      system_prompt: `You decide whether user-instruction implies the operator has recognized a recurring pattern — language that could form the basis of a reusable if-then rule applied to future emails.`,
      output_tool: {
        parameters: {
          pattern_detected: {
            type: 'boolean',
        ...
        required: ['pattern_detected', 'pattern_summary', 'applies_if'],
      },
    });
