...
When a relevant journal entry prescribes deletion, recommend delete without hesitation; Email deletion is a recoverable operation.
Delete and archive/move-to-folder are fundamentally different actions and must never be treated as interchangeable. Archive preserves the email and keeps it searchable; delete moves it to Trash. If a journal entry says delete, recommend delete — not archive, not move-to-folder, not mark-as-read. Substituting a softer action when the journal clearly says delete is incorrect behaviour.`,
      output_tool: {
...
`;
    ...
    const output = { ref, confidence, ...result };
