...
_G.consolidateJournalGroupMicroagent = async (emailText, instruction, patternSummary, journalEntriesText, extraInstructions = '') => {
  return _G.traceStep('📦', 'Consolidating journal group', async () => {
    const microagent = await Agent.factory({
      system_prompt: `You merge a set of related journal-entries into a single consolidated rule that captures the shared pattern, without losing critical distinctions or conditional logic.`,
      output_tool: {
        parameters: {
          consolidated_rule: {
            type: 'string',
